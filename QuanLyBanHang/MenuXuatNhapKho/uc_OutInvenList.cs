    ...
    public partial class uc_OutInvenList : UserControl
    {
        ...
        private void LoadData()
        {
            try
            {
                ...
                dgv_DanhSachHoaDon.DataSource = hd.GetHoaDon("");
                FormatGrid();
            }
            catch (Exception ex)
            {
        ...
        private void dgv_DanhSachHoaDon_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                if (e.ColumnIndex == dgv_DanhSachHoaDon.Columns["Detail"].Index)
                {
                    ...
                    hd.LoadChiTiet();
                    uc_OutInvenCreate uc = new uc_OutInvenCreate(hd, true);
                ...
                if (e.ColumnIndex == dgv_DanhSachHoaDon.Columns["Fix"].Index)
                {
                    ...
                    hd.ConLai = Convert.ToDecimal(dgv_DanhSachHoaDon.CurrentRow.Cells["ConLai"].Value);
                    hd.LoadChiTiet();
                    uc_OutInvenCreate uc = new uc_OutInvenCreate(hd, false);
        ...
        private void txt_ThongTinTimKiem_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                HoaDon hd = new HoaDon();
                dgv_DanhSachHoaDon.DataSource = hd.GetHoaDon(" and (hd.MaHoaDon like '%" + txt_ThongTinTimKiem.Text + "%' or hd.KhachHang like '%" + txt_ThongTinTimKiem.Text + "%' or kh.TenKhachHang like N'%" + txt_ThongTinTimKiem.Text + "%' or hd.NhanVien like '%" + txt_ThongTinTimKiem.Text + "%' or nv.TenNhanVien like N'%" + txt_ThongTinTimKiem.Text + "%' or CONVERT(VARCHAR(10),hd.NgayXuat,103) like '%" + txt_ThongTinTimKiem.Text + "%')");
                FormatGrid();
            }
            catch (Exception ex)
            {
